...
export async function tryReconnectOnce(): Promise<ReconnectOutcome> {
  ...
  try {
    ...
    if (resolveConnectionMode(record) === 'cloud_connector') {
      // Desktop local API can still talk to the LAN even if an older bug
      // saved the device as cloud_connector.
      if (!env.deviceSyncEnabled) {
        return {
          connected: false,
    ...
    try {
      ...
      if (test.online) {
        const workingPort = test.resolvedPort ?? record.port;
        if (workingPort !== record.port) {
          await updateDeviceAddress(record.id, record.ip_address, workingPort);
        }
        ...
        console.log(
          `[Device] Compatible device found and authenticated: ${record.ip_address}:${workingPort} (${info.model ?? record.brand})`,
        );
        return {
          connected: true,
          ipAddress: record.ip_address,

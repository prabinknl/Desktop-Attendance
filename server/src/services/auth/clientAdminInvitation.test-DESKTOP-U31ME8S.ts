...
describe('Client Admin Invitation & SMS Verification', () => {
  ...
  describe('Invitation Record Workflow', () => {
    ...
    it('resolves an owner-provided 6-digit code via sms_code_hash without exposing other invitations', async () => {
      const rawToken = generateSecureToken();
      const smsCode = '482917';
      await InvitationModel.save({
        token: rawToken,
        token_hash: hashValue(rawToken),
        sms_code_hash: hashValue(smsCode),
        email: 'new.admin@client.com',
        role: 'client_admin',
        company_name: 'Pace Consultant',
        created_at: new Date().toISOString(),
        expires_at: new Date(Date.now() + INVITATION_TTL_MS).toISOString(),
        sms_expires_at: new Date(Date.now() + SMS_CODE_TTL_MS).toISOString(),
        used: false,
        status: 'pending',
      });

      const found = await InvitationModel.getByCode(smsCode);
      expect(found?.matchedBy).toBe('sms');
      expect(found?.inv.email).toBe('new.admin@client.com');
      expect(found?.inv.role).toBe('client_admin');
    });

    it('resolves a raw invitation token for a client/admin invite', async () => {
      const rawToken = generateSecureToken();
      await InvitationModel.save({
        token: rawToken,
        email: 'owner.invited@client.com',
        role: 'client',
        created_at: new Date().toISOString(),
        expires_at: new Date(Date.now() + INVITATION_TTL_MS).toISOString(),
        used: false,
      ...
      const found = await InvitationModel.getByCode(rawToken);
      expect(found?.matchedBy).toBe('token');
      expect(found?.inv.email).toBe('owner.invited@client.com');
    });

    it('does not treat accountant invitations as admin invitations', () => {

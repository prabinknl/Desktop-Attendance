...
export async function sendAdminVerificationEmail(input: {
  ...
  if (!smtpConfigured()) {
    if (env.nodeEnv !== 'production') {
      ...
      console.log(`[Auth DEV MODE] Verification code for ${recipients}: ${input.code}`);
      console.log('==========================================================\n');
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      to: input.to,
      subject: 'Owner / Admin verification code',
      text: [
    ...
    if (env.nodeEnv !== 'production') {
      console.warn(`[Auth DEV MODE] SMTP send failed (${message}); dev fallback active — check server logs for the code.`);
      return { sent: true, devFallback: true };
    }

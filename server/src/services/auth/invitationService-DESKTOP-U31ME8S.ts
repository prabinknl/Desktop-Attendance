...
/** Owner-created invitations that may register an Admin account. */
const ADMIN_INVITATION_ROLES = new Set(['admin', 'client', 'client_admin']);

export function isAdminInvitationRole(role: string | undefined | null): boolean {
  return ADMIN_INVITATION_ROLES.has(String(role ?? '').trim().toLowerCase());
}

/** Prefill display name from the invited email when the invitation has no name field. */
export function displayNameFromInviteEmail(email: string): string {
  const local = String(email ?? '').split('@')[0] ?? '';
  const spaced = local.replace(/[._-]+/g, ' ').replace(/\s+/g, ' ').trim();
  if (!spaced) return 'Admin';
  return spaced.replace(/\b\w/g, (char) => char.toUpperCase());
}

export function publicAdminInvitePayload(inv: InvitationRecord) {
  return {
    email: inv.email,
    name: displayNameFromInviteEmail(inv.email),
    companyName: inv.company_name || undefined,
  };
}

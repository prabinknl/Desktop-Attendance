...
function saveToDisk() {
  try {
    ...
    const payload = JSON.stringify(
      { device: memoryDevice, logs: memoryLogs, invitations: memoryInvitations, users: memoryUsers, savedAt: new Date().toISOString() },
      null,
      2,
    );
...
export const memoryStore = {
  ...
  getInvitationBySmsCodeHash(smsCodeHash: string): InvitationRecord | null {
    ...
    return memoryInvitations.find((i) => i.sms_code_hash === smsCodeHash) || null;
  },
  ...
  upsertUser(user: MemoryUser): MemoryUser {
    const email = user.email.trim().toLowerCase();
    const saved: MemoryUser = { ...user, email };
    memoryUsers = [saved, ...memoryUsers.filter((u) => u.email.trim().toLowerCase() !== email)];
    saveToDisk();
    return saved;
  },

  findUserByCredentials(identifier: string, password: string): MemoryUser | null {
    const key = identifier.trim().toLowerCase();
    return (
      memoryUsers.find(
        (u) =>
          u.password === password &&
          (u.email.trim().toLowerCase() === key || u.name.trim().toLowerCase() === key),
      ) || null
    );
  },
};

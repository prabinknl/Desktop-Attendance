...
export const authApi = {
  ...
  login: async (identifier: string, password: string) => {
    const { data } = await apiClient.post<{ success: boolean; data?: CloudUser }>(
      '/auth/login',
      { identifier, password },
      { timeout: 8000, validateStatus: (status) => status === 200 || status === 401 },
    );
  ...
  verifyAdminInvite: async (input: { code: string; phone: string }) => {
    const { data } = await apiClient.post<{
      success: boolean;
      message?: string;
      data?: { email: string; name: string; companyName?: string };
    }>('/auth/admin/verify-invite', input, {
      validateStatus: (s) => s === 200 || s === 400 || s === 404 || s === 410 || s >= 500,
    });
    return data;
  },

  registerAdminWithInvite: async (input: { code: string; phone: string; name: string; password: string }) => {
    const { data } = await apiClient.post<{
      success: boolean;
      message?: string;
      data?: CloudUser;
    }>('/auth/admin/register-invite', input, {
      timeout: 8000,
      validateStatus: (s) => s === 200 || s === 400 || s === 404 || s === 409 || s === 410 || s >= 500,
    });
    return data;
  },

...
export default function PunchRequestPanel() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const emp = empMap[watchEmployeeId];
      if (!emp) {
        setDayRecord(null);
        return;
      }
  ...
  }, [watchEmployeeId, watchDate, empMap]);

  const filtered = useMemo(() => requests.filter((r) => {
    if (user?.role === 'employee' && user.employeeId && r.employeeId !== user.employeeId) return false;
  ...
  }), [requests, empMap, search, statusFilter, user]);
  ...
  const openForm = () => {
    ...
    const defaultEmp = user?.role === 'employee' && user.employeeId
      ? (employees.find((e) => e.id === user.employeeId || e.employeeId === user.employeeId)?.id ?? '')
      : '';
    reset({
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">{requests.length} total requests</p>
        </div>

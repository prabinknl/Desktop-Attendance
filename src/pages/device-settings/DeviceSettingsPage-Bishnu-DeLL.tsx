...
function statusLabel(status?: string, authState?: string, gatewayStatus?: string): string {
  if (gatewayStatus === 'offline' || authState === 'gateway_offline') return 'Gateway Offline';
  if (authState === 'device_unreachable') return 'Device Unreachable (LAN)';
  if (authState === 'authentication_failed') return 'Authentication Failed';
  if (authState === 'isapi_unsupported') return 'ISAPI Endpoint Unsupported';
...
export default function DeviceSettingsPage() {
  ...
  const scanColumns: ColumnsType<DiscoveredDevice> = [
    ...
    },
    {
      ...
      key: 'action',
      render: (_, row) => (
        <Button type="link" onClick={() => handleConnectFromScan(row)}>
          Use IP
        </Button>
      ),
    },
      ...
      <div className="device-settings-page">
        ...
        >
          <Row gutter={[24, 16]} align="middle">
              ...
              <Space size="middle">
                ...
                <div>
                  <Title level={4} style={{ margin: 0, color: isOnline ? '#059669' : '#dc2626' }}>
                    {statusLabel(status?.status, testResult?.authState, status?.gatewayStatus)}
                  </Title>
            ...
            <Col xs={24} md={16}>
              <Row gutter={[16, 8]}>
                <Col span={8}>
                  <Text type="secondary">Local Gateway</Text>
                  <div>
                    <Tag color={status?.gatewayStatus === 'online' ? 'success' : 'error'}>
                      {status?.gatewayStatus === 'online' ? 'Agent Online' : 'Gateway Offline'}
                    </Tag>
                  </div>
                </Col>
                <Col span={8}>
        ...
        {status?.gatewayStatus !== 'online' && (
          <Alert
            ...
            message="Local Gateway / Sync Agent Required"
            description={
              <div>
                <p className="mb-2">
                  The physical Hikvision machine (<strong>{device?.ipAddress || '192.168.0.6'}</strong>) is on your local office LAN.
                  The cloud-hosted app syncs via a local gateway script running on your office computer.
                </p>
                <div className="font-mono text-xs bg-slate-900 text-emerald-400 p-3 rounded-lg my-2">
                  cd gateway && node index.js
                </div>
                <p className="text-xs text-slate-600">
                  Run <strong>register-startup.bat</strong> inside the <code>gateway/</code> folder to start automatically on Windows boot.
                </p>
              </div>
            }
          />

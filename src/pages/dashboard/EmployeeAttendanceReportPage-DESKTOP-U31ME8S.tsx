...
export default function EmployeeAttendanceReportPage() {
  ...
  const canEditManualTime = user?.role === 'admin' || user?.role === 'account' || user?.role === 'hr' || can('attendance:write');
  ...
  const handleSaveManualTime = async (
    record: Attendance,
    field: 'manualCheckIn' | 'manualCheckOut',
    value: string,
  ) => {
    if (!canEditManualTime) {
      toast('error', 'Permission Denied', 'Only Admin or Account users are authorized to edit attendance times');
      return;
    }

    const timeVal = value.trim() ? (value.length === 5 ? `${value}:00` : value) : undefined;
    const isSynthetic = isGapAttendanceRow(record) || record.id.startsWith('leave-row-');
    const editorName = user?.name || user?.email || 'Admin';
    const nowIso = new Date().toISOString();
    
    const nextManualIn = field === 'manualCheckIn' ? timeVal : record.manualCheckIn;
    const nextManualOut = field === 'manualCheckOut' ? timeVal : record.manualCheckOut;

    // Star only the side being edited; clearing the value removes its star.
    const edited = Boolean(timeVal);
    const editFields = field === 'manualCheckIn'
      ? {
          checkInEdited: edited,
          checkInEditedBy: edited ? editorName : undefined,
          checkInEditedAt: edited ? nowIso : undefined,
        }
      : {
          checkOutEdited: edited,
          checkOutEditedBy: edited ? editorName : undefined,
          checkOutEditedAt: edited ? nowIso : undefined,
        };
    
    const effIn = nextManualIn || record.checkIn;
    const effOut = nextManualOut || record.checkOut;
    let computedHours = record.workingHours || 0;
    if (effIn && effOut) {
      const [hi, mi] = effIn.split(':').map(Number);
      const [ho, mo] = effOut.split(':').map(Number);
      if (!Number.isNaN(hi) && !Number.isNaN(ho)) {
        computedHours = Math.max(0, Math.round(((ho * 60 + mo - (hi * 60 + mi)) / 60) * 100) / 100);
      }
    }
    ...
    try {
      if (isSynthetic) {
        ...
        const newRecord = await AttendanceAPI.create({
          ...
          departmentId: employee?.departmentId || record.departmentId || 'd0',
          date: record.date,
          shiftId: employee?.shiftId || record.shiftId || 's1',
          checkIn: record.checkIn,
          checkOut: record.checkOut,
          manualCheckIn: nextManualIn,
          manualCheckOut: nextManualOut,
          ...editFields,
        ...
        setAttendance((prev) => [newRecord, ...prev.filter((p) => p.date !== record.date)]);
      } else {
        const updated = await AttendanceAPI.update(record.id, {
          [field]: timeVal || (null as any),
          ...editFields,
          workingHours: computedHours,
          manualOverride: true,
          status: record.status === 'absent' ? 'present' : record.status,
        });
        setAttendance((prev) => prev.map((p) => (p.id === record.id ? updated : p)));
      }
      toast('success', 'Manual time saved');
    } catch (err) {
      toast('error', 'Failed to save manual time', err instanceof Error ? err.message : 'Error');
    }
  };
